...
> **화면 크기는 이제 자동으로 정해진다**(2026-09-16). 예전에는 1280×720을 고정으로
> 요청했는데 **실제로는 640×480으로 열리고 있었다** — 요청이 통하지 않았는데 아무도
> 몰랐다. 지금은 처음 한 번 큰 크기부터 열어 보며 **크기와 속도를 같이 재서** 가장 좋은
> 것을 고르고, 그 값을 저장해 다음 실행부터는 건너뛴다(같은 PC·같은 웹캠 실측:
> 640×480 → **1920×1080**, 둘 다 초당 30장).
>
> 큰 화면이 항상 좋은 건 아니다 — 같은 웹캠이 2560×1440에서는 **초당 1장**밖에 못 냈다
> (손 동작을 따라갈 수 없다). 그래서 초당 15장(`RTAUTO_VISION_CAMERA_MIN_FPS`)을 못
> 넘기는 크기는 더 크더라도 버린다.
>
...
> 화면 크기가 바뀌었으면 손 보정(`calibrate_dg5f.py`)을 다시 하는 편이 안전하다.
