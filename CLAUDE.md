...
## 원칙 6 — 학습이 끝나면 사용자가 눈으로 확인한다

숫자만으로 끝내지 않는다. **학습이나 큰 실험이 끝나면 사용자가 직접 화면으로 보고
판단한다** (2026-09-14 이후 관행, 2026-09-17 사용자 요청으로 규칙화).

- 학습·평가가 끝나면 **눈으로 볼 수 있는 방법을 같이 안내한다** — 어떤 명령을 어느
  터미널에서 돌리면 되는지까지(원칙 3). 숫자만 보고하고 끝내지 않는다.
- 볼 수단이 없으면 **만들어서 준다**(예: `superdex/scripts/watch_grasp.py`,
  `arm/watch_arm.py`).
- **사용자가 눈으로 본 것은 숫자만큼 중요한 증거다.** 2026-09-17 에 사용자가 화면에서
  "손가락이 종이컵을 뚫고 들어간다" 를 발견했다. 숫자만 보던 쪽에서는 못 찾던 것이고,
  이 관찰이 물리 접촉을 실측하게 만들었다.
- ⚠️ **다만 눈으로 본 것을 성급히 결함으로 결론내지도 않는다.** 위 건은 조사 끝에
  **문제가 아니었다** — 파고드는 깊이 0.59 mm 는 정상 범위이고, 시뮬레이터 안 컵의
  벽 두께는 **2.2 mm** 였다. 내가 **현실 종이컵 값(0.3~0.5 mm)과 비교**해 결함으로
  단정했던 것이 오류다. **실물 값으로 시뮬레이터를 판단하지 말고, 시뮬레이터 안의
  값을 재고 나서 판단한다.**
