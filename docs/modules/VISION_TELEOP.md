...
**증상**: 새끼손가락을 옆으로 벌려도 로봇 새끼가 따라오지 않는다(사용자 보고, 2026-09-16).
같은 불만이 2026-07-20에도 있었고 그때는 절반만 고쳤다(`pinky_lat` 게이트 해제).

**원인 두 가지** — 근거는 `logs/calib_log_20260911_165841.csv`(702프레임)와
`logs/vision_dg5f_20260916_094347.csv`(3,091프레임) 실측:

| # | 원인 | 숫자 |
|---|---|---|
| ① | **로봇에 보내는 창이 너무 좁다** — 사람 새끼 좌우 폭 102°를 24° 창(±12°)에 통과시킨다 | 보정 기록의 **78.5%가 창 밖**(검지 2.3%, 약지 3.3%). 텔레옵 기록에서 **34%가 정확히 +12°에 붙어** 있었고 중앙값이 +10.3° |
| ② | **좌우값에 굽힘이 섞인다** — 그래서 ①의 창을 일부러 좁혀 놨던 것 | 새끼 좌우값 vs 새끼 굽힘 상관 **−0.77**(중지 0.00, 검지 +0.71, 약지 −0.56) |

**②의 기술적 원인**: 옛 계산(`_abduction_planar`)은 손가락 첫 마디 방향을 **손바닥 평면에
눌러 붙인 뒤** 중지 방향과의 각도를 쟀다. 손가락을 굽히면 그 그림자가 짧아져 같은 흔들림이
훨씬 큰 각도로 증폭된다(수학적으로 나쁜 조건).

**바꾼 것**: `_abduction_lateral` — 첫 마디 **단위벡터의 옆 방향 성분**을 arcsin으로 각도로
바꾼다. 굽힘은 '앞↔손바닥' 평면에서 일어나 옆 성분을 거의 바꾸지 않는다.
`dg5f_angles.ABDUCTION_METHOD`로 옛 방식(`"planar"`)을 다시 켤 수 있다(비교·재현용).

| 검증 | 방법 | 결과 |
|---|---|---|
| 가상 손(정답을 아는) | `tests/test_abduction.py` — 벌림·굽힘 각도를 넣어 21개 점을 직접 만든다 | 많이 굽힌 구간의 흔들림: 옛 방식 1.10°→**10.52°**(9.5배 악화), 새 방식 1.08°→**2.77°**(2.6배). 쭉 편 손가락에서는 두 방식이 1° 안에서 일치(부호 규약 유지 확인) |
| 실제 손 | `analyze_abduction.py` — `probe_landmarks.py`로 녹화한 기록에서 두 방식을 대조 | **사용자 녹화 대기 중** |

⚠️ **새 방식의 알려진 성질**: 손가락을 굽힐수록 읽히는 벌림각이 cos(굽힘각)만큼 **작아진다**
(쭉 편 상태에서는 정확히 일치, 다 굽히면 0). 옛 방식처럼 폭주하는 대신 예측 가능하게
작아지는 쪽을 택한 것이다.

⚠️ **①(창 넓히기)은 아직 적용하지 않았다.** ②를 실제 손 기록으로 확인한 뒤, 그 기록의
사람 값 분포를 보고 `DG5F_CHANNELS`의 `pinky_lat` 범위를 정한다. 기구 한계는
−15°~+90°로 여유가 크다([`DG5F_JOINT_RANGES.md`](DG5F_JOINT_RANGES.md) §2).
