...
**텔레옵**은 사람이 움직이면 로봇이 그대로 따라 하게 하는 것이다. 여기서는 **웹캠 앞에서 손을
움직이면 로봇 손이 같은 모양을 만든다.**

들어가는 것은 웹캠 영상, 나오는 것은 **로봇 손 관절이 가야 할 각도**와 손끝 위치 정보다.
사람 손에서 **마디마디의 위치를 찾아내는 일**은 MediaPipe 라는 라이브러리가 하고, 그 위치를
로봇 명령으로 바꾸는 계산은 파이썬이 한다. Unity 는 받은 각도를 화면 속 관절에 그대로 적용한다.

이건 **스스로 물체를 찾아 잡는 학습과는 완전히 별개의 길**이다. 웹캠을 켜 놨다고 해서 학습
프로그램이 물체 위치를 카메라에서 받아 보게 되는 게 아니다. 카메라 여러 대를 쓰는 기능도
**웹캠 한 대로 하는 이 손동작 조작에 자동으로 붙지 않는다.**
...
| 파일 | 성격 |
|---|---|
| `vision_node_dg5f.py` | **기본으로 쓰는 쪽.** 웹캠 화면을 띄워 보여 준다. 조절 패널은 없지만 **화면 없이 도는 프로그램은 아니다**(창이 하나 뜬다). 실행파일(.exe)로 묶는 대상이 이쪽이다 |
| `dg5f_teleop_gui.py` | **조절 패널이 달린 쪽.** 보낼 주소와 번호, 변환 방식, 사람 손 보정값, 관절 한계를 **돌아가는 중에** 바꿔 볼 수 있다. 실행파일로 묶는 대상은 아니다 |

`dg5f_teleop_gui.py` 는 **일을 셋으로 쪼개 동시에 돌린다** — ① 웹캠에서 영상 받기 ② 손 인식하고
각도로 바꿔 보내기 ③ 화면 그리기. 각 단계 사이에는 **가장 최근 것 하나만 남겨 두는 자리**를 두고
밀린 영상은 버린다. 예전에 이걸 한 줄로 이어 붙였더니, **조절 막대를 움직여도 밀린 영상이 다
처리될 때까지 화면이 굳던 문제**가 있어서 이렇게 고쳤다.
...
| `one_euro_filter.py` | **손떨림을 지우는 부품.** 천천히 움직일 때는 많이 다듬어 부드럽게 하고, 빠르게 움직일 때는 덜 다듬어 **반응이 늦지 않게** 한다. 관절마다 따로 하나씩 둔다 |
| `joint_ranges.py` | 사람과 로봇의 관절 가동범위 대조표. 매핑 구현의 정본은 `dg5f_angles.py`이며 전 채널이 동일한 1:1 변환은 아님 |
| `dg5f_paths.py` | 기록 파일과 보정 파일을 **어디에 어떤 이름으로 둘지 정하는 한 곳.** 실행할 때마다 새 파일 이름을 만들어(초 단위, 겹치면 뒤에 번호) **전에 받아 둔 기록을 덮어쓰는 사고를 막는다** |

### 패킷 계약 (버전이 붙어 있다)

**숫자를 줄줄이 이어 붙인 덩어리**를 보낸다. 숫자 하나가 4바이트이고, 바이트를 늘어놓는 순서가
정해져 있다(`float32 little-endian` — 받는 쪽도 같은 규약으로 읽어야 한다).

받는 쪽은 **적어도 앞의 20개는 읽고, 덩어리가 길면 뒤까지 더 읽는다.** 손 브리지는 앞의 20개만
쓴다. 받는 프로그램을 새로 만들 때도 **이 "길이에 따라 더 읽는다"는 규칙을 지켜야 한다.**
...
실제 관절 명령은 앞의 `[0..19]`(도 단위)뿐이다. 뒤쪽 값들은 **손 크기로 나눠서 0~1 근처로 맞춰
놓은 방향 값**이라, 로봇 치수에 맞춰 손끝이 갈 곳을 다시 계산할 때 쓴다 — **그대로 미터 좌표로
읽으면 안 된다.**

Unity 에는 **손끝이 갈 목표 위치를 주면 관절 각도를 거꾸로 계산해 주는 기능**이 있다. 그 기능이
켜진 손가락은 그쪽이 움직이고, 각도를 그대로 넣는 방식은 비켜 준다 → [UNITY.md](UNITY.md).
...
| `analyze_thumbik.py` | 엄지 손끝 계산이 남긴 기록을 분석한다. 어긋난 이유가 **① 방향이 반대인지 ② 값이 통째로 밀렸는지 ③ 명령은 맞는데 물리적으로 못 따라간 건지 ④ 애초에 팔이 닿지 않는 곳인지**를 갈라 준다 |
...
| ⑤ | `compute_thumb_tip` / `compute_finger_tips` / `compute_wrist_tip_vectors` | 손끝이 어느 방향으로 얼마나 뻗었는지를 **손 크기로 나눠 0~1 근처 값**으로 만든다. Unity 에서 **손끝 위치로 관절 각도를 거꾸로 계산할 때** 넣는 값이다 |
...
| 카메라가 여러 대면 | `RTAUTO_VISION_CAMERA_INDEX` = **`ask`**(기본, 선택 창) / 숫자(고정) / `auto`(자동). 번호 고정일 때 이번만 고르려면 `vision_node_dg5f.py --pick`. 목록은 `camera_caps.py --list` |
...
**선택 창 (`ask` / `--pick`)**: `camera_picker.py`가 tkinter(파이썬 기본 포함)로 띄운다 —
새로 설치할 것이 없다. 목록에 번호·크기·속도를 쓰고, 스캔할 때 각 카메라에서 받아 둔 **사진
한 장**을 함께 보여 준다(어느 쪽이 외장 웹캠인지 눈으로 확인). 사진은 Pillow가 있을 때만
표시되고 없으면 글자 목록만 나온다. 카메라가 한 대면 묻지 않고 바로 시작하며, 창을 띄울 수
없는 환경(화면 없는 리눅스)에서는 터미널로 번호를 묻는다. 취소하면 **아무 카메라도 열지
않고 끝낸다** — 엉뚱한 카메라로 조용히 시작하지 않기 위해서다.
**시작 비용**(이 PC 실측, 카메라 1대): 번호 고정 3.2초 / `ask` 4.5초 / `auto` 4.5초.
찾는 단계에서는 **크기·속도를 설정하지 않고 열기만 한다**(2.6초 → 1.4초). 크기·속도는
저장해 둔 값을 보여 주고, **처음 보는 카메라만** 그 자리에서 제대로 재서 저장한다 —
안 그러면 "둘 다 640x480"으로 보여 어느 쪽이 좋은지 알 수 없다.

⚠️ **찾는 동안 카메라를 붙들고 있으면 안 된다** (2026-09-16 실측). "찾을 때 연 카메라를
그대로 쓰면 한 번 덜 열 텐데" 싶어 그렇게 만들어 봤더니 반대였다 — 카메라 하나를 연 채로
다음 번호를 확인하면 없는 번호 판정이 0.1초에서 3초로 느려져 **전체가 2.7초 → 19초**가 됐다.
그래서 찾을 때는 하나씩 열고 **반드시 닫는다**.
