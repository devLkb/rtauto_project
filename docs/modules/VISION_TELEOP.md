...
| 카메라가 여러 대면 | `RTAUTO_VISION_CAMERA_INDEX`에 번호 / **`auto`**(가장 좋은 것 자동) / **`ask`**(선택 창). 실행할 때만 고르려면 `vision_node_dg5f.py --pick`. 번호 확인은 `camera_caps.py --list` |
...
사진을 받을 때는 카메라를 640x480으로 잠깐만 여는데, 여러 대를 동시에 열 때 USB 대역폭을
덜 쓰려는 것이다(실제 실행은 다시 최적 크기로 연다).
