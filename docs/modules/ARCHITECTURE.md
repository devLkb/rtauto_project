...
- **값의 정본은 항상 코드다.** 그림에 적힌 번호·숫자는 `.env` 가 없을 때의 기본값이며,
  그림과 코드가 다르면 코드가 맞다(원칙 1 — 설정은 `config/rtauto_config.py` + `.env`).
- 2026-09-07 에 저장소의 코드·씬 설정을 읽어서 맞춘 것이다. **실물로 다시 시험했다는 뜻이 아니다.**
  화살표는 "이렇게 이을 수 있다"는 뜻이지 "전부 동시에 켜라"가 아니다.

> 🔶 **2026-09-16 정정.** 아래 그림들은 **2026-09-07** 기준이다. 이틀 뒤인 **2026-09-09 에
> 물체를 잡는 학습이 Unity 에서 SuperDex 로 옮겨갔다** —
> [`SUPERDEX_POC_PLAN.md`](../SUPERDEX_POC_PLAN.md). 그래서 §4 "강화학습 루프"와 그림 속
> 학습 상자는 **Unity 에서 학습하던 때의 배선**으로 읽는다. 나머지 그림(통신 번호, 누가 로봇을
> 움직일 권한을 갖는지, 자산 만드는 순서, 설정)은 지금도 그대로 맞다.

## 범례

| 표기 | 뜻 |
|---|---|
| 실선 화살표 | 지금 만들어져 있고 실제로 쓰는 연결 |
| 점선 화살표 | **옵션을 줘야 켜지거나**, 아직 안 되는 부분이 있는 연결 |
| 파랑 상자 | 지금 쓰는 것 |
| 주황 상자 | **아직 안 되는 부분이 있어서** 다 됐다고 보지 않는 것 |
| 회색 점선 상자 | 예전 것 · 폐기된 것 · 남아 있는 잔재 |

### 그림 안에 영어로 적힌 말

그림은 코드에 있는 이름을 그대로 써야 찾을 수 있으므로 영어를 남겼다. 뜻은 아래와 같다.

| 그림의 말 | 뜻 |
|---|---|
| **PhysX** | Unity 가 쓰는 **물리 계산 부분**의 이름. 무게·부딪힘·마찰을 여기서 계산한다 |
| **PPO** | 학습 방식의 이름. 점수를 보고 프로그램을 **조금씩만** 고쳐 나간다 |
| **headless** | **화면을 띄우지 않고** 도는 실행 방식. 학습할 때는 그림을 그릴 필요가 없어 이게 훨씬 빠르다 |
| **ONNX (.onnx)** | 학습이 끝난 프로그램을 **파일 하나로 저장한 형식** |
| **IK** | 손끝이 갈 **위치를 주면 관절 각도를 거꾸로 계산**해 주는 기능 |
| **RTDE** | UR 팔과 **실시간으로 값을 주고받는 통신 방식**의 이름 |
| **SDK** | 장비 제조사가 준 **장비 조작용 프로그램 묶음** |
...
```mermaid
flowchart BT
  BASE["KDT.RobotScripts<br/>Assets/Scripts — 통신·구동·손끝계산·기록"]
  PNP["KDT.PicknPlaceTraining<br/>Unity 안의 학습 코드"]
...
| 경계 | 상태 |
|---|---|
| 웹캠 → Unity 손 | **된다.** 학습과는 상관없는 별도 길이다 |
| Unity → 실물 손 | **된다.** `Dg5fSender` 는 **기본이 꺼짐**이라 일부러 켜야 움직인다(안전) |
| 실물 손 → Unity | ⚠️ **아직 안 된다.** 사람이 손으로 직접 자세를 잡는 동안에는 브리지가 각도를 읽어 보내는 부분까지 가지 못한다 ([REAL_BRIDGES §2](REAL_BRIDGES.md)) |
| Unity ↔ URSim 팔 | 양방향 코드와 과거 검증 기록 있음. **실물 UR16e 재검증과는 구분** |
| 스스로 잡는 데모 | ⚠️ 저장된 데모 화면에서 **해당 부품이 꺼져 있고**, 자동으로 바꾸는 코드가 그걸 다시 켜 주지 않는다 ([UNITY §0](UNITY.md)) |
| 카메라 여러 대로 3차원 위치 잡기 | 따로 실행하고 따로 맞춰야 한다. 손동작 조작이나 학습에 자동으로 붙지 않는다 |
| 카메라로 물체를 찾아 실물에서 스스로 잡기 | **아직 시작도 안 했다.** 실물에서 값을 얻는 방법, 카메라와 로봇의 좌표 맞추기, 검증이 전부 남아 있다 |
| 손에 두 프로그램이 동시에 붙기 | **안 된다.** DG-5F 는 어떤 방식으로 접속하든 **한 번에 한 프로그램만** 받는다(제조사 프로그램 DGManager 를 켜 두면 브리지가 붙지 못한다) |
