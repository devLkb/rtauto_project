...
| 3D 비전 | **고정 웹캠 3대**(전역 위치·장애물) + **손목 장착 eye-in-hand**(파지 직전 정밀) **2단 구성** | 두 캘리브레이션이 성격이 다르다 — 고정 카메라는 **eye-to-hand**(§5-7), 손목은 **eye-in-hand**(§2). eye-in-hand 카메라는 **Intel RealSense D405로 확정**(2026-09-14, v18) — 실물 장착 전까지는 보유 웹캠으로 대체 |
...
```text
   UR16e 컨트롤박스 ─RJ45─┐
                          ├─[ TP-Link ES205G ]─RJ45─(LANStar LS-UCLAN-3M)─ PC ─USB─ 웹캠 3대
...
| **3** | 웹캠 기반 실시간 조작 재검증 | 신규 웹캠 3대 적용해 재검증 + 팔 동작 동시 기록 → 모방학습 데이터 확보 | **텔레옵 기 구현** — **보류** (v14). 모방학습 데이터 확보도 선택지로 강등 |
| **4** | 손목 카메라 (eye-in-hand) | **Intel RealSense D405**(확정)를 손목에 장착. 기존 MediaPipe용 웹캠은 용도 변경. ★ **Camera↔base 캘리브레이션(§5-7)이 선행 조건** | 신규 — **보류** (v14, 장착 대기) |
...
| 센서 | 담당 구간 |
|---|---|
| 고정 웹캠 3대 | 물체 대략 위치, 장애물 인식 → 경로계획 |
| eye-in-hand (손목) | 파지 직전 정밀 자세 추정 |
...
> 상태이므로(v14 작업 범위 제한), 그때까지는 보유 웹캠을 손목에 장착해 진행하고 깊이
> 정보가 없는 구간은 ArUco 마커와 고정 카메라 삼각측량으로 보완한다. 실물 검증 착수 시
> D405로 교체한다.
