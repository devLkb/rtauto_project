...
팔·AMR 없이 **그리퍼 단독**으로 Unity ↔ 실물을 왕복 연결하고, 위 1~4번 항목의 상당 부분을
실측으로 확정했다. 확정된 사실은 `vision/dg5f/dg5f_sdk_bridge.py` 상단 주석이 정본이다.

| 항목 | 결과 |
|---|---|
| 관절 대응 (`JOINT_ORDER`) | **항등 확정** — 하드웨어 설명서 §3.3.1의 Motor N ↔ 우리 채널 N-1 |
| 부호 (`JOINT_SIGN`) | **전 채널 +1 확정** (아래 함정 참고) |
| 영점 (`JOINT_OFFSET_DEG`) | 편 상태에서 전 채널 ±5° 이내 → **0 유지** |
| 가동 범위 (`JOINT_CLAMP`) | 뭉뚱그린 ±130 → **채널별 실제 범위**(설명서 §3.3.1) |
| PID 게인 | **연결할 때마다 설정 필수** — 안 넣으면 전 관절 무응답. P=2.0(매뉴얼 §11.2 예제값) |
| SDK 콜백 | `ConnectToGripper` **전에 등록 필수** — 안 하면 네이티브 access violation |
| 상태 피드백 | `GetReceivedGripperData`로 관절각·전류·모듈에러 수신(부채③ 해소) |

> ⚠️ **함정 기록 — 설명서 §3.3.1의 범위 표는 부호 없는 이동량이다.** 표의
> "Motor 2 (0° to 155°)"를 부호 있는 범위로 읽고 "URDF(-155..0)와 거울"이라 판단해
> `thumb_opp`의 부호를 뒤집었는데 **오판이었다.** 실물 엔코더·명령 프레임은 URDF와 같은
> 음수 대향 규약이다. 뒤집은 탓에 Unity의 대향 명령이 반대편 하드 스톱(실측 +3.7°)으로
> 나가 **1.5 A로 스톨**했고, 증상은 "엄지가 아예 안 움직인다"로 보였다. 무응답이 아니라
> 반대로 밀고 있던 것이다. **전류를 함께 보면 즉시 갈린다**(0 mA=미구동 / 1.5 A=밀고 있음)
> — 이후 진단은 반드시 전류를 켜고 할 것.

**도구** (모두 `vision/dg5f/`):
- `dg5f_sdk_bridge.py` — Unity/비전 → 실물 구동. `--jog IDX:DEG`(관절 단독 구동+전류),
  `--track CSV`(명령 vs 실제 기록·요약), `--echo-to-unity`(실물 상태를 트윈에 반사)
...
| ~~관절 대응 미검증~~ → **2026-08-31 해소** (순서·부호·영점·범위 실측 확정, §5 참고) | ~~하드웨어 손상~~ | 완료. 단 **부호 오판으로 하드 스톱 스톨(1.5 A)을 겪었으므로**, 새 관절/새 하드웨어에서는 반드시 전류를 켜고 `--jog`로 단독 검증할 것 |
