...
| | 단계 | 내용 | 상태 |
|---|---|---|---|
| **1** | 로봇팔 디지털 트윈 | URSim으로 소프트웨어 완성 → 실물 연결 시 IP 교체. `ur_calibration` 추출 | **착수 가능 (하드웨어 불필요)** |
| **2** | 팔·손 통합 트윈 | 그리퍼를 5포트 이더넷 스위치에 연결, PC가 팔·손 동시 소유 | **손 코드 있음**, 연결만 |
| **3** | 웹캠 기반 실시간 조작 재검증 | 신규 웹캠 3대 적용해 재검증 + 팔 동작 동시 기록 → 모방학습 데이터 확보 | **텔레옵 기 구현**, 재검증·확장 |
| **4** | 손목 카메라 (eye-in-hand) | 3D 비전 카메라를 손목에 장착. 기존 MediaPipe용 웹캠은 용도 변경. ★ **Camera↔base 캘리브레이션(§5-7)이 선행 조건** | 신규 |
| **5** | 관찰 재설계 및 재학습 | 기존 강화학습의 입력값을 프로젝트에 맞게 재설계하고 학습 | 신규 |
