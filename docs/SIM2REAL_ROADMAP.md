...
- `MinimumSpawnRadius`/`MaximumSpawnRadius` 0.35/0.55→**0.37/0.58**: 리치 비율
  ...
  같은 상대 난이도를 유지하는 쪽을 택함. 검증된 `ur16e_dg5f_right.sim.mjcf.xml`
  기준 FK로 `ArmSafeMinDeg/MaxDeg` 봉투 안에서 wrist_3 수평 반경이 대략
  0.07~0.91 m까지 나오는 걸 확인했으므로 새 애뉼러스는 봉투 안쪽에 여유롭게
  들어간다.
...
- `ArmSafeMinDeg`/`ArmSafeMaxDeg` — **변경 없음, 단 리스크 하나 발견**. URDF
  ...
  값들은 리치가 아니라 UR5e 기하에서 손으로 튜닝된 "그럴듯한 자세" 봉투다.
  UR16e sim MJCF로 64개 코너 전부 FK 스윕한 결과, 한 코너
  (shoulder_lift=-20°, elbow=140°)에서 손이 패널 상판(z=0)보다 약 0.20 m 아래로
  내려간다 — UR5e에서도 원래 있던 문제인지 비교할 동일 방법론의 UR5e sim
  모델이 없어 판단 보류, 값은 바꾸지 않음. 실제 학습 실행 또는 UR5e sim
  모델 비교로 재확인 전까지는 추측으로 좁히지 않는다.
- 액추에이터 게인(더 무거운 팔) — `patch_mjcf.py`에 1차값(kp=2000/kv=200)
  있음, Unity xDrive 게인과의 정량 대응은 미완료.
...
동일한 건전성 지표를 통과했다. ~~inertial 없는 UR 더미 링크 6개는 Unity 임포트
또는 MJCF 변환 단계에서 보정이 필요하다~~ → **실측으로 반증됨, 아래 "MuJoCo
로드·구동 검증 완료" 참고.**
...
#### MuJoCo 로드·구동 검증 완료 (2026-08-25)

`ur16e_dg5f_right.urdf`를 MuJoCo Python(`mujoco` 패키지, `vision/.vision` venv)에
직접 로드하고, `urdf/ur16e_dg5f_right_build/patch_mjcf.py`로 URDF가 표현 못 하는
요소(26개 position 액추에이터, 손끝·팜 터치 센서, 파지 대상 블록, 자기충돌 처리)를
추가해 `ur16e_dg5f_right.sim.mjcf.xml`을 만들었다. 5000스텝(10 s) 구동으로
팔 준비포즈 + 손 쥐기포즈 추종을 검증: 추종오차 0.1° 이내, NaN 없음, 자기충돌
오검출 없음, 블록이 바닥에 안정적으로 정지.

이 과정에서 로드맵의 예측 두 가지가 실측과 달랐다:

1. **관성 누락 UR 더미 링크 패치는 불필요했다.** MuJoCo는 URDF 컴파일 시 fixed
   joint로 연결된 질량 0 바디를 부모 바디에 자동으로 병합한다(41 링크 → 27
   MuJoCo 바디). `base_link`/`base`/`flange`/`tool0`/`ft_frame`에 수동으로
   mass/inertia를 채워 넣는 작업(Unity/WORKLOG §22 방식)은 MuJoCo에서 재발하지
   않는다.
2. **자기충돌 제외는 쌍 목록이 아니라 그룹 방식으로 대응됐다.**
   `unity/Assets/Scripts/RobotSelfCollisionIgnore.cs`를 직접 확인한 결과, 특정
   링크 쌍을 고르는 게 아니라 로봇 전체 콜라이더 쌍을 `Physics.IgnoreCollision`로
   블랑켓 비활성화하는 방식이었다. MuJoCo에서는 이를 3-그룹 `contype`/
   `conaffinity` 비트마스크(로봇↔로봇 무충돌, 로봇↔바닥/물체는 충돌)로 동치
   구현했다 — 쌍 목록을 옮겨 적는 작업이 아니었다.

추가로 명시적(Euler) 적분기에서 손가락 등 관성이 작은 링크가 QACC 발산을
일으켜, 적분기를 `implicitfast`로 전환해 해결했다. 액추에이터 게인
(팔 kp=2000/kv=200, 손 kp=20/kv=2)은 안정성만 확인한 1차 추정치이고, 기존
Unity xDrive 게인(10000/200/100000)과의 정량 대응은 아직 하지 않았다.
...
- **액추에이터** — 26관절 position 서보. **완료(1차)** — `patch_mjcf.py`가 팔
  kp=2000/kv=200, 손 kp=20/kv=2로 추가, 10 s 구동에서 추종오차 0.1° 이내로
  안정 확인. Unity xDrive 게인(10000/200/100000)과의 정량 대응은 아직 안 함
- **터치 센서** — 핑거팁·팜. **완료** — MuJoCo 네이티브 touch 센서로 추가, 실물
  FT 센서와 1:1 대응
- **자기충돌 처리** — **완료** — Unity `RobotSelfCollisionIgnore`는 쌍 목록이
  아니라 로봇 전체를 블랑켓 무시하는 방식임을 확인, MuJoCo에서 3-그룹
  `contype`/`conaffinity` 비트마스크로 동치 구현(로봇↔로봇 무충돌, 로봇↔바닥/
  물체는 충돌)
- **마찰·솔버 파라미터**(solref/solimp) — MuJoCo로 가는 이유 자체. 기본값 사용
  중, 실물 마찰 실측(Phase 1) 전까지는 임시값
- ~~**관성 누락 링크 패치**~~ — **불필요로 확인됨.** MuJoCo가 fixed joint로
  연결된 질량 0 바디를 부모에 자동 병합하므로(41 링크 → 27 바디) UR 더미 링크
  5개에 대한 수동 mass/inertia 패치가 필요 없다. 위 "MuJoCo 로드·구동 검증
  완료" 참고
- **대상 물체** — 블록(지금, **완료** — 0.035×0.035×0.12 m, density 1800, COM
  0.20 위치로 `Dg5fGraspLiftSpec` 기본값과 일치) / 웨이퍼 케이스(나중, 스펙 대기)
...
**Python MuJoCo 쪽은 절반 완료(2026-08-25):** `ur16e_dg5f_right.sim.mjcf.xml`이
MuJoCo Python에서 로드·구동됨을 5000스텝(10 s) 실행으로 증명했다(위 "MuJoCo
로드·구동 검증 완료" 참고). 이는 B안이 성립 가능함을 보인 것이지, 아래 세 항목
(Unity 플러그인 쪽)을 대신하지 않는다 — A/B 결정에는 여전히 이 셋이 필요하다.

1. **플러그인 × Unity 6000.4 호환** — 튜토리얼 기준은 >2021.2.19f다. 우리 에디터에서
   임포트·스텝이 도는지부터 확인. **미착수** — `unity-cli`가 이 머신 PATH에 없어
   프로그램적으로 에디터를 구동하지 못함. 에디터를 직접 열어 확인하거나
   `unity-cli` 설치/경로 확보가 필요.
2. **처리량** — 단일 `MjScene`에 학습영역 20개를 올려 스텝 레이트 측정.
   현재 Unity PhysX 구성과 비교. **미착수**
3. **크기 랜덤화 비용** — geom 크기 변경 시 씬 재생성 실측. 비싸면 커리큘럼을
   런 단위 고정 또는 영역별 배치로 재설계. **미착수**
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| `ArmSafeMinDeg/MaxDeg` 봉투가 손을 패널 아래로 내리는 코너 존재 (FK 확인) | 학습 중 비현실적/위험 자세 방문 가능 | 실제 학습 실행 관찰 또는 UR5e sim 모델과 비교 후 필요시 봉투 축소 |
