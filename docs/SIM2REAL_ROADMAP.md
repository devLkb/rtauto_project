...
| 3D 비전 | **고정 웹캠 3대**(전역 위치·장애물) + **손목 장착 eye-in-hand**(파지 직전 정밀) **2단 구성** | 두 캘리브레이션이 성격이 다르다 — 고정 카메라는 **eye-to-hand**(§5-7), 손목은 **eye-in-hand**(§2). Orbbec Gemini 305는 구매 승인 대기 중이라 승인 전까지 보유 웹캠으로 대체 |
| 실물 연결 | 컨트롤박스 RJ45 **1개**(무선랜 없음) → **이더넷 스위치 단일 구성**. 개발은 URSim 선행 | v7(2026-09-03) — RS-485 단기안 폐기. §5 "결정 (2026-09-03 최종)"·"URSim 선행 개발 경로". 무선 구간 경계는 §10 |
...
추가 비용: 5포트 스위치 1개(~1만원), 랜케이블 2개(현 보유 1개 + 2), 노트북에 유선
포트가 없으면 USB-이더넷 어댑터 1개(~1.5만원).
...
| **2** | 팔·손 통합 트윈 | 그리퍼를 5포트 이더넷 스위치에 연결, PC가 팔·손 동시 소유 | **손 코드 있음**, 연결만 — **보류** (v14, 실물 제약) |
| **3** | 웹캠 기반 실시간 조작 재검증 | 신규 웹캠 3대 적용해 재검증 + 팔 동작 동시 기록 → 모방학습 데이터 확보 | **텔레옵 기 구현** — **보류** (v14). 모방학습 데이터 확보도 선택지로 강등 |
| **4** | 손목 카메라 (eye-in-hand) | 3D 비전 카메라를 손목에 장착. 기존 MediaPipe용 웹캠은 용도 변경. ★ **Camera↔base 캘리브레이션(§5-7)이 선행 조건** | 신규 — **보류** (v14, 장비·장착 대기) |
...
> **Orbbec Gemini 305는 구매 승인 대기 중**(2026-09-03). 승인 전까지는 보유 웹캠을
> 손목에 장착해 진행하고, 깊이 정보가 없는 구간은 ArUco 마커와 고정 카메라 삼각측량으로
> 보완한다. 승인 시 손목 카메라를 Gemini 305로 교체한다.
