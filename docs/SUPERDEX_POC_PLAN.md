...
| **다음 착수점** | 게이트 0~3 통과, **게이트 4 진행 중**(DR 스윕 완료, 2단계 DR 재학습 실행 중). 🛑 **이 브랜치의 범위는 게이트 4까지 = 엔진 선택 종료**다 — 그 뒤 확장(손목 자세·들기·UR16e 통합·FOUP 규모)은 **별도 승인 필요**. §5 "작업 경계" 참고 |
...
**다만 완전한 일반화는 아니다.** 배치를 맞춰도 70 % 로 duck_lamp 의 100 % 에 못 미친다.
정책이 duck_lamp 하나로만 학습됐으니 예상되는 결과다 — 여러 물체를 섞어 학습하면
올라갈 여지가 있으나, 그것은 **엔진 판단에 필요한 실험이 아니다**(§5 "작업 경계").
