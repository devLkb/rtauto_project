...
> # 📌 2026-09-10 인수인계 — 새 컨텍스트는 이 블록만 읽어도 이어받을 수 있다
>
> **하루 사이에 세 가지가 바뀌었다: 개발 머신, 아키텍처 승인, 그리고 act26 구현.**
>
> ### 1. 개발 환경이 개인 노트북(Linux)으로 옮겨졌다
> 작업 머신이 2대(Windows)에서 **3대**가 됐다. 새 머신은 LG gram 16 `16ZD90TP-GX56K` —
> Core Ultra 5 225H (14C/14T), **CUDA 없음**(Intel Arc 내장), **RAM 16 GB**,
> Ubuntu 24.04.4. 상세와 대응은 이 문서 **§8** 이 정본이다.
> - `.env` 에 `RTAUTO_SUPERDEX_GPUS_PER_LEARNER=0` 이 필요하다(코드 기본값 1 은 CUDA 전제).
> - torch 는 **CPU 휠**(`2.14.0+cpu`). `torch.cuda.is_available()=False` 가 정상이다.
> - **성능 판정 기준은 여전히 회사 머신이다.** 이 노트북 수치를 게이트 판정에 쓰지 마라.
> - 부트스트랩 전 절차는 [`PYTHON_ENV_SETUP.md`](PYTHON_ENV_SETUP.md) **부록 A-1~A-6**.
>   `project_superdex` `stable`(`1d71509`) 클론이 **필수**다(공식 asset 이 wheel 에 없다).
>
> ### 2. teacher-student 아키텍처가 **승인**됐다 (사용자 지시)
> [`GRASP_POINT_ARCHITECTURE.md`](GRASP_POINT_ARCHITECTURE.md) 의 권고가 채택됐고
> [`RL_POLICY_REDESIGN.md`](RL_POLICY_REDESIGN.md) 가 **v6** 으로 개정됐다.
> 바뀐 것 3가지: 후보 M=8 은 teacher 필수 입력에서 제외 / student 는 점군 직접 입력이
> 정상 / **GT 유출 금지는 배포(student) 정책에만 적용**(teacher 특권 상태는 의도된 1단계).
> ⚠️ **계약 문서 v6 부터는 "구현하지 않는 제안 문서"가 아니다** — 헤더의 구현 현황 표를 보라.
>
> ### 3. act26 이 구현됐다 (1단계 teacher 확장의 첫 항목)
> `superdex/envs/dg5f_grasp_env.py` 에 **`control_mode`** 가 생겼다:
>
> | 모드 | 관찰/행동 | ONNX 계약 | 용도 |
> |---|---|---|---|
> | **`arm_hand26`** (기본값) | **77 / 26** | `dg5f-grasp-2` | 게이트 3 결합 bot. 팔6 + 손20 |
> | `hand20` | 65 / 20 | `dg5f-grasp-1` | **게이트 2·4 재현 전용.** 과거 체크포인트 호환 |
>
> - 액션 배치는 계약 §5-2 표 그대로 — `0..5` 팔6(`shoulder_pan`…`wrist_3`), `6..25` 손20.
> - **절대 목표각**이다(계약 서술의 "델타"와 다르다 — 기존 손20 구현 규약을 팔로 확장했다).
> - 베이스는 용접(root DOF 0). 프로젝트 동작 원칙(정지 후 매니퓰레이션)과 일치한다.
> - 게이트·평가 스크립트 9종은 전부 `control_mode="hand20"` 을 명시해 **과거 재현이 안 깨진다.**
>   실측 확인: 게이트 2 기준선 파지 성립 6/6, 게이트 3 손 장착 1.49e-08 통과.
> - 검증: `python superdex/scripts/smoke_act26_isolation.py` — 팔/손 분리 구동을 수치로 판정.
>
> ### 🛑 다음 착수점 — 여기서 이어가라
> 1. **`place` 상수 제거 — 메커니즘은 구현됐다. 남은 건 학습 검증이다.**
>    `spawn_mode="workspace"` 로 물체를 지면 위 무작위 위치에 놓을 수 있고(사람 상수 없음),
>    `spawn_radius_frac` 이 커리큘럼이다. 안착높이·도달범위는 **실측**하며 낙하 판정과
>    접근 shaping(potential-based)까지 함께 넣었다 — 아래 "workspace 스폰" 절 참고.
...
> 3. ✅ **물체 다양성 확보 — 끝났다 (2026-09-14). 로스터가 3종 → 15종이 됐다.**
>    `shape_box` 도형 12종에 오라클 스윕을 돌려 **전부 잡히는 것을 실측**했다(8~10/10 시드).
>    외부 물체 셋(YCB) 도입은 불필요해졌다. 아래 "shape_box 오라클 스윕" 절이 정본이다.
>    - 재현: `python -u superdex/scripts/object_oracle_sweep.py` (약 40분)
>    - ⚠️ **덤으로 게이트 2의 "2.5 cm 는 기하적으로 불가" 판정이 흔들렸다** — 배치 후보를
>      3개로 넓히자 `block_red` 가 1/10 → **6/10** 이 됐다. 기하가 아니라 **배치**였다.
>    - 👉 **다음은 섞어 학습이다.** 12종을 리셋마다 바꿔 가며 teacher 를 학습시키고,
>      학습에 쓰지 않은 도형으로 넘어가는지(홀드아웃) 본다. throughput 이 여기서 병목이 된다.
> 4. **점군 합성 프로토타입** (teacher-student 2단계). 아직 실측 전인 최대 미검증 항목이다.
>
> ### ⚠️ 이어받는 사람이 알아야 할 함정
> - **공식 DG5F asset 은 `defaultPose` 로 손가락이 굽은 자세를 갖고 배포된다**(20관절 전부
>   비영: 15°, −55° …). 리셋 자세를 정할 때 의식하지 않으면 **손을 반쯤 쥔 채 에피소드가
>   시작된다.** 이것 때문에 게이트 3 검증이 한 번 오진했다 — 아래 게이트 3 절 U9 블록 참고.
> - **throughput 이 다음 단계의 정면 리스크다**(실효 ≈470 steps/s). 물체 다양성 + 26 DOF +
>   DR 이 겹치면 여기서 막힌다. 노트북은 CPU learner 라 더 느리다.
> - 아직 **별도 승인이 필요한 범위**가 남아 있다: 실물 규모, sim2real, 손목 자세 액션.
...
| **다음 착수점** | **`place` 상수 제거** (물체별 사람 튜닝 상수 → 자율 결정). teacher-student 승인 완료(2026-09-10), act26 구현 완료 — 위 "2026-09-10 인수인계" 블록 참고 |
...
| **3** UR16e 결합 | ✅ **통과 (2026-09-09), Linux 재확인 (2026-09-10)** | Studio bake 완료. **구운 UR16e 팔 + 공식 DG5F 손 결합체**가 로드·구동된다 — REVOLUTE 26 = 팔 6 + 손 20, 팔 관절 한계 UR16e 스펙 일치, `tool0` 생존, **홈 자세 자기접촉 0**(계측기 자기검사로 검증), 5초 정지 수렴, 중력 처짐 0.98 mm, **손 장착 회전이 정본 URDF와 일치**(최대 차이 1.49e-08) |
...
| 3 | 팔+손 결합체가 성립하는가 | **예.** REVOLUTE 26, 자기접촉 0, 손 장착이 정본 URDF 와 **1.49e-08** 일치 — 2026-09-10 개인 노트북(Linux)의 새 클론에서 **독립 재확인**(U9 해소, 검증 스크립트의 `default_pose` 정규화 버그를 고친 뒤) |
...
> ✅ **U9 — 해소됨 (2026-09-10). 게이트 3 판정은 유효하다. 스크립트에 재현성 버그가
> 있었고 고쳤다.**
>
> 개인 노트북(Ubuntu 24.04)에서 `project_superdex` `stable`(`1d71509`)을 새로 클론해
> 검증하니 위 표가 재현되지 않고 **최대 차이 8.18e-02 (8 cm)** 가 나왔다. 추적한 결과
> **로봇·asset·마운트 문제가 아니라 비교 방법의 결함**이었다.
>
> **원인 — 서로 다른 자세를 비교하고 있었다.** 공식 Tesollo asset
> (`dg5f_long_right.superdex_bot`)은 `defaultPose` 필드로 **손가락이 굽은 자세**를 갖고
> 배포된다 — 20 관절 **전부 비영**(15°, −55°, −6°, 2°, 5° …). URDF 에는 defaultPose 개념이
> 없어 0 자세로 로드된다. `hand_pose_in_tool0()` 이 "홈 자세"를 각 모델의 기본 자세로
> 잡는 바람에 **A는 굽은 손, B는 편 손**을 비교했다.
>
> **증명:** `prefab.default_pose` 를 0 으로 맞추고 다시 재면 **최대 차이 1.49e-08** —
> 2026-09-09 에 기록된 값과 **정확히 같은 수치**로 떨어진다. 즉 그때의 비교 조건이
> 복원된 것이고, 두 손 기하가 실제로 동일함이 **Linux 에서 독립적으로 재확인**됐다.
>
> | 조건 | 엄지끝 (tool0) | 최대 차이 |
> |---|---|---|
> | `default_pose` 그대로 (공식 굽은 자세) | `[0.1076, 0.0671, 0.1287]` | 8.185e-02 |
> | **`default_pose` = 0 (정규화)** | `[0.0258, 0.1251, 0.0866]` | **1.490e-08** |
> | B 정본 URDF | `[0.0258, 0.1251, 0.0866]` | — |
>
> **고친 것:** `gate3_verify_hand_mount.py` 의 `hand_pose_in_tool0()` 이 비교 전에
> `default_pose` 를 0 으로 정규화한다. 이 버그는 **새 머신이면 어디서든 터졌을 것**이므로
> (원칙 2 위반) 스크립트 쪽을 고치는 것이 맞다.
>
> **배제된 오해 2가지 — 기록해 둔다.**
> - **장착 회전은 처음부터 옳았다.** 손바닥 위치·축은 두 경우 모두 차이 0 이었다.
>   ⚠️ 그런데도 스크립트는 "`SUPERDEX_HAND_MOUNT_QUAT` 를 고쳐야 한다"고 결론지었다 —
>   **불일치를 무조건 마운트 탓으로 돌리는 오진**이다. 손가락 자세 차이는 마운트
>   쿼터니언으로 고칠 수 없다. `identity` 는 확정값이며 건드리지 않는다.
> - **asset 해시가 머신마다 다른 것은 정상이다.** 회사 머신 실측은
>   `1ABE9…0E787F`, Linux 클론은
>   `cc42f60f01a51f40a0e02daec74fa319f580edaa0b6e6c6a1b3fd079ef468ac7` 로 다르다. 이 파일은
>   **LF-only JSON 텍스트인데 공식 리포에 `.gitattributes` 가 없어서** Windows 체크아웃이
>   CRLF 로 바뀐다 — 바이트는 달라도 파싱 결과는 같다. 실제로 기하가 1.49e-08 로 일치하는
>   것이 그 증거다. **이 해시 차이를 asset 불일치의 근거로 쓰지 마라.**
>
> **부수 소득:** 공식 asset 이 손가락 굽힘 기본자세로 배포된다는 사실은 파지 환경에서
> 리셋 자세를 정할 때 알아야 한다 — `defaultPose` 를 그대로 쓰면 손이 반쯤 쥔 상태에서
> 에피소드가 시작된다.

### act26 환경 확장 (2026-09-10) — 팔6 + 손20 착수

**결정:** `Dg5fGraspEnv`의 새 기본 `control_mode="arm_hand26"`은 게이트 3에서 통과한
구운 UR16e + 공식 DG5F 결합 bot을 쓴다. 행동은 절대 관절 목표각 26개다. 정책 문서의
델타 제안으로 임의 전환하지 않았다 — 게이트 1의 RLlib `normalize_actions`/ONNX unsquash
경로와 현행 손20 환경이 이미 절대 목표각을 계약으로 쓰기 때문이다.

- 행동: **26 = 팔 6 (`0..5`) + 손 20 (`6..25`)**. 관절 한계는 새 숫자를 적지 않고 결합
  prefab의 REVOLUTE 한계에서 읽는다.
- 관찰: 기존 손20 65개를 앞에 보존하고, 끝에 팔 각6·각속도6을 붙여 **77**개가 된다.
- 베이스: root를 `HARD`로 용접한다. 매니퓰레이션은 베이스 정지 상태에서만 한다는
  `CLAUDE.md` 원칙을 물리 제약으로도 강제한다.
- 공식 손의 `defaultPose`는 20관절 모두 굽어 있으므로, 결합 모드에서만 관절 0 자세로
  정규화한다. 과거 게이트 2·4 정책은 `control_mode="hand20"`으로 손 단독 65/20 계약을
  그대로 재현한다.

**실제 CPU smoke (2026-09-10):**

터미널 1 (bash, 리포 루트, `superdex/.venv` 활성):

```bash
source superdex/.venv/bin/activate
python -u superdex/scripts/smoke_act26_isolation.py
```

출력: root DOF=0, 팔 액션만에서 팔 최대 **2.524384 rad** / 손 최대
**1.213459e-04 rad**, 손 액션만에서 손 최대 **9.397004e-01 rad** / 팔 최대
**7.482322e-05 rad**로 분리 판정을 통과했다 (각 120 스텝).

ONNX 계약은 **`dg5f-grasp-2`**(obs77/action26)로 올렸다. `dg5f-grasp-1`은 hand20
호환용 과거 체크포인트 export에 남긴다. 새 act26 체크포인트와 Unity 3자 파리티는 아직
없으므로, 이 기록은 환경·구동 smoke의 통과이지 새 정책 학습/배포 통과 선언이 아니다.

### workspace 스폰 (2026-09-10) — `place` 상수 제거의 기반

**문제.** `Dg5fGraspEnv` 는 물체 스폰 위치를 `place`(기본 `(0.04, 0.0, 0.04)`)로 잡는데,
이건 **손바닥 기준 offset** 이라 물체가 언제나 파지 영역 안에 나타난다. 즉 정책이
**접근을 배울 필요가 없다.** 게다가 이 값은 물체마다 사람이 맞춘 상수다 — 게이트 4 에서
`paper_cup` 은 배치만 바꿔 성립률 30 % → 100 % 로 뛰었다. 목적이 범용 파지로 바뀐 뒤
(로드맵 v17) 이건 편의가 아니라 **결함**이고, 인수인계 §0 이 최우선 착수점으로 지목한 항목이다.

**구현.** `spawn_mode` 스위치를 넣었다. 팔 6축이 선행 조건이라 `arm_hand26` 에서만 쓸 수 있다.

| `spawn_mode` | 배치 | 용도 |
|---|---|---|
| **`palm`** (현재 기본값) | 손바닥 기준 `place` offset | 게이트 0·2·4 재현. **기존 실측 조건 보존** |
| `workspace` | **지면 위 무작위**, 사람 상수 없음 | 팔이 스스로 접근. 범용 파지의 전제 |

**숫자를 타이핑하지 않았다(원칙 1).** workspace 모드가 필요로 하는 두 값을 **실측**한다:

| 값 | 방법 | duck_lamp/UR16e 실측 |
|---|---|---|
| 물체 안착 높이 | 로봇에서 떨어뜨려 정착시킨 뒤 기준점 z 를 읽는다 (접촉 침투까지 반영된 값이라 `get_aabb_local()` 로 대신할 수 없다. 당시 "AABB 를 못 읽는다"고 적었던 것은 틀렸다 — 2026-09-11 정정) | **0.0291 m** |
| 팔 도달 범위 | 관절 한계 안에서 무작위 자세 400개를 FK 로 풀어 베이스 기준 수평거리의 분위수 | 샘플링 **[0.298, 0.874] m** / 실측 최대 **1.094 m** |

> 도달 상한 0.874 m 가 **UR16e 스펙 리치 900 mm 와 일치**하는 것이 이 측정이 옳다는
> 교차검증이다. 실측 최대 1.094 m 가 그보다 큰 것은 파지중심이 플랜지가 아니라 **손 끝단**
> 이기 때문이다(손 길이가 더해진다).
>
> ⚠️ 분위수 상한(`_reach_hi`)과 실측 최대(`_reach_max`)를 혼동하지 마라. 전자는 특이점
> 부근을 피한 **샘플링용**이고, "도달 가능한가" 판정에는 후자를 써야 한다 — 팔 영점 자세
> (거의 완전 신전, 0.925 m)가 분위수 상한을 넘지만 당연히 도달 가능한 자세다.
> 이 구분을 안 해서 smoke 판정이 한 번 오탐했다.

**커리큘럼.** `spawn_radius_frac` ∈ [0,1] 이 "팔 시작 자세 바로 아래"(0)에서 "도달범위
전체"(1)까지 연속으로 넓힌다 — 계약 §5-3 의 "초기에는 위치를 가까이 두어 손20 의 유효
조절부터 배운다"를 이 값으로 구현한 것이다. 실측 스폰거리 중앙값: 0.043 / 0.458 / 0.905 m.

**먼 스폰이 깨뜨리는 것 두 개를 함께 고쳤다.**

1. **낙하 판정.** 기존 `dropped = dist > 0.25` 는 절대 상수라, 물체가 25 cm 밖에 스폰되면
   중력이 켜지는 즉시 종료되고 −5.0 을 받는다 — **학습이 시작조차 못 한다.** workspace
   모드에서는 `max(drop_margin, 스폰거리 + drop_margin)` 로 **스폰 거리 기준 상대값**을 쓴다.
2. **사라지는 gradient.** `r_reach = exp(−10·d)`, `r_near = exp(−8·d)` 는 50 cm 에서 사실상
   0 이다. 그렇다고 밀집 항을 그냥 더하면 **v6 에서 겪은 보상 해킹 채널**이 다시 열린다
   (손을 물체 근처에 두기만 해도 리턴을 벌었다). 그래서 **potential-based shaping**
   (Ng et al. 1999, Φ=−dist)을 쓴다 — `F = γ·Φ(s′) − Φ(s)` 는 할인 리턴에 주는 영향이
   `−Φ(s₀)` 하나로 닫혀 **최적 정책을 바꾸지 않는다는 것이 증명돼 있다.**

   > ⚠️ **2026-09-14 수정 — 처음 구현은 이 조건을 지키지 않았다.** `F = Φ(s′) − Φ(s)`
   > (γ=1)로 넣었는데 PPO 는 `gamma=0.99` 로 학습한다. 정리의 불변성은 **두 γ 가 같을
   > 때만** 성립하고, 어긋난 만큼 `(1−γ)·Φ(s′) = −0.01·dist` 라는 **거리 벌점**이 스텝마다
   > 남는다. 크기는 작지만 "최적 정책을 바꾸지 않는다"는 근거 자체가 성립하지 않았다.
   > 함께 고친 것이 하나 더 있다 — **종료(흡수) 상태의 퍼텐셜은 0 이어야 한다.** 낙하로
   > 끝나는 스텝에서 `Φ(s′)=−dist` 를 그대로 쓰면 shaping 합이 닫히지 않아 **멀리서
   > 떨어뜨리는 쪽에 유리한 편향**이 남는다(시간 초과는 흡수 상태가 아니므로 해당 없음).
   > 할인율 정본은 `config/rtauto_config.py` 의 `SUPERDEX_PPO_GAMMA` 하나이고 학습기와
   > 환경이 같은 값을 읽는다(원칙 1). 검증은 `smoke_workspace_spawn.py` 가 수치로 한다 —
   > shaping 을 켠 할인 리턴과 끈 할인 리턴의 차이가 이론값 `−Φ(s₀) + γᵀ·Φ(s_T)` 와 일치.
   `palm` 모드에서는 가중치가 **0 이 기본**이라 기존 조건이 한 톨도 바뀌지 않는다.
...
안착높이 유한·지면 z 분산 0, 스폰이 실측 최대도달 이내, 커리큘럼 단조 증가
(0.043 → 0.458 → 0.905 m), **중력 직후 즉시 낙하종료 0/10**, palm 기본값 보존까지 통과한다.

**회귀 없음 — 커밋본과 소수점 9자리까지 동일.** `hand20`·`arm_hand26` 각각 3 에피소드
결정론적 롤아웃의 누적 보상이 이 변경 전후로 완전히 같다
(`hand20 [239.528540584, 105.252563474, 105.222465028]`,
`arm_hand26 [89.891282109, 32.116089517, 40.722145796]`). 신규 경로가 전부
`spawn_mode == "workspace"` 뒤에 게이트돼 있기 때문이다.

> 🛑 **기본값은 아직 `palm` 이다 — 학습으로 검증하지 않았다.**
> 이 작업이 만든 것은 **메커니즘**이고, "정책이 실제로 접근을 배우는가"는 **학습을 한 번
...
### 물체 다양성 전수 조사 (2026-09-10)

범용 파지로 목적이 바뀐 뒤 성공 조건은 "학습하지 않은 물체로 넘어가는 것"이다. 그런데
게이트 4 에서 쓴 물체는 4종뿐이었다. **공식 asset 트리에 무엇이 있는지부터 수치로**
알아야 해서 전수 조사 도구를 만들었다.

```bash
python superdex/scripts/survey_object_assets.py
```

**측정 방법.** 프리팹이 참조하는 **충돌 메시(`shape`, `.mochi.h5` 의
`mesh/coordinates`)를 직접 읽고** `scale` 을 곱한다. 렌더 메시(.glb)는 fallback 이다.
물리 초기화가 필요 없어 빠르다.

> 정정 (2026-09-11): 처음에 "런타임에서는 AABB 를 못 읽는다"고 적었는데 **틀렸다** —
> `actor.get_aabb_local()`·`get_rigid_center_of_mass_local()` 이 런타임에서 읽힌다.
> 아래 "shape_box 오라클 스윕" 절에서 이 API 로 **root 가 메시 원점**임을 밝혔다.

> 이 방법이 옳다는 검증: 측정값이 문서에 기록된 실측값과 **일치**한다 —
> `duck_lamp 11.9×7.5×11.0 cm`, `paper_cup 9.3×9.3×11.3 cm`, `block 2.5 cm`, `sphere 3.0 cm`.
>
> ⚠️ **처음엔 렌더 메시(.glb)만 봤다가 duck_lamp 을 통째로 놓쳤다.** 이 물체는
> **`soft`(변형체) actor** 라 `renderModel` 필드가 아예 없다. actor 종류를 `rigid` 로
> 한정하면 **주력 학습 물체가 조사에서 사라진다.**

**결과 — 쓸 수 있는 서로 다른 형상은 3종뿐이다.**

| 판정 | 서로 다른 형상 | 내용 |
|---|---:|---|
| ✓ 적합 | **3** | `duck_lamp`(soft, 11.9 cm), `paper_cup`(9.3 cm), `shapebox_body`(14.0 cm, 사실상 용기) |
| ? 미검증 구간 | **14** | **`shape_box` 의 도형 12종**(cross·triangle·square·trapezoid·rectangle·pentagon·parallelogram·octagon·hexagon·ellipse·diamond·star, 3.8~5.0 cm) + peg board + sphere |
| ✗ 불가 | — | 2.5 cm 블록 4색(게이트 2 실측 근거), 9-hole peg(0.6 cm), chain(0.2 cm), 손보다 큰 조립 씬 |

> **인스턴스 수에 속지 마라.** `paper_cup_pyramid` 의 컵 11개는 **전부 같은 컵**이다.
> 도구가 AABB 로 중복을 접어서 세는 이유다 — 접기 전 "적합 12개"가 접으면 3종이 된다.

**"미검증 구간"을 불가로 단정하지 않았다 — 이게 이번 조사의 핵심이다.**
당시 실측으로 아는 것은 두 점뿐이었다: 2.5 cm 는 게이트 2에서 성립하지 않았고,
**9.3 cm·11.9 cm 는 성립했다**. 그 사이 3~5 cm 구간은 **아무도 측정하지 않았다.**
하나의 임계값으로 잘라 버리면 `shape_box` 의 12가지 도형을 근거 없이 버리게
된다 — 그건 서로 형상이 다르고 크기가 고른, **일반화 학습에 가장 알맞은 집합**이다.

> ✅ **2026-09-14 — 측정됐다. 단정하지 않은 것이 옳았다.**
> `shape_box` 도형 12종이 **전부 잡힌다**(8~10/10 시드). 위 표의 "미검증 14종" 중 12종이
> 여기서 해소되고 로스터가 **3종 → 15종**이 된다. 남은 미검증은 peg board 와 sphere 둘.
>
> "✗ 불가"로 적은 **2.5 cm 블록도 배치를 넓히면 6/10** 이라, 그 칸의 근거(게이트 2)는
> 기하가 아니라 배치 조건이었다. 조사 도구의 임계값도 이 실측에 맞춰 갱신했다
> (최소변 4.5 cm 기준 → 최대변 4.0 cm 기준. `rectangle` 은 **최소변 2.7 cm 인데 9/10 이다**).
> 상세는 아래 "shape_box 오라클 스윕 (2026-09-14)" 절.

### shape_box 오라클 스윕 (2026-09-14) — 도형 12종이 **전부 잡힌다**

전수 조사가 "미검증 구간"으로 남긴 `shape_box` 도형 12종(3.8~5.0 cm)을 실제로 쟀다.
방법은 `gate2_oracle_search.py` 와 같다 — **학습하지 않고** 개루프 폐쇄 궤적을 그리드로
훑어 각 시드에 성공 궤적이 **존재하는지** 센다. 학습으로 재면 실패했을 때 물체 탓인지
학습 탓인지 갈리지 않는다.
...
그리드는 폐쇄율 3 × 램프 2 × 조임여유 2 × **배치 3**(손바닥 앞 0.02/0.03/0.04 m) = 30 궤적,
시드 10개(9000~9009). 대상 15개에 60만 스텝, 약 40분(개인 노트북).

**결과 — 12종 전부 통과.** 판정 경계는 게이트 2와 같다(≥70 % 잡힌다 / ≤30 % 어렵다).

| 도형 | 크기 (cm) | 질량 | 성공 시드 | 최선 고정궤적 | tips 중앙/최대 | 낙하 |
|---|---|---:|---:|---:|:---:|---:|
| cross | 3.8×4.0×3.8 | 19.5 g | **10/10** | 6/10 | 3 / 4 | 41 % |
| triangle | 4.6×4.0×3.9 | 14.0 g | **8/10** | 5/10 | 3 / 4 | 67 % |
| square | 3.2×4.0×3.3 | 16.5 g | **10/10** | 7/10 | 3 / 4 | 58 % |
| trapezoid | 4.0×4.0×3.1 | 15.6 g | **10/10** | 6/10 | 3 / 3 | 42 % |
| rectangle | 3.7×4.0×**2.7** | 16.0 g | **9/10** | 7/10 | 3 / 3 | 42 % |
| pentagon | 4.2×4.0×3.9 | 17.7 g | **10/10** | 7/10 | 3 / 4 | 40 % |
| parallelogram | 4.7×4.0×**2.7** | 16.2 g | **10/10** | **9/10** | 3 / 4 | 30 % |
| octagon | 3.7×4.0×3.7 | 17.7 g | **9/10** | 6/10 | 3 / 4 | 45 % |
| hexagon | 4.1×4.0×3.6 | 17.5 g | **10/10** | 8/10 | 3 / 4 | 43 % |
| ellipse | 4.5×4.0×3.2 | 17.5 g | **10/10** | 8/10 | 3 / 4 | 41 % |
| diamond | 3.9×4.0×5.0 | 15.4 g | **10/10** | 8/10 | 3 / 4 | 46 % |
| star | 4.3×3.6×4.1 | 13.6 g | **10/10** | 8/10 | 3 / 4 | 39 % |

**로스터가 3종 → 15종이 됐다**(기존 `duck_lamp`·`paper_cup`·`shapebox_body` + 도형 12종).
서로 형상이 다르고 크기가 고른 집합이라 일반화 학습에 그대로 쓸 수 있다. 외부 물체
셋(YCB 등) 도입은 **지금은 불필요하다**.

> ⚠️ **조사 도구의 임계값을 이 실측으로 갱신했더니, 재본 적 없는 물체가 하나 따라
> 올라왔다.** 9홀 페그 보드(12.5×12.5×**4.0** cm)가 새 크기 기준을 통과한다 —
> 판정이 "최소변 4.5 cm" 에서 "최대변 4.0 cm + 최소변 2.7 cm" 로 바뀌었기 때문이다.
> **잡아 본 적이 없으므로 로스터 15종에 넣지 않았다.** 도구도 이제
> **"실측"과 "크기 기준만 통과"를 구분해 찍는다** — 임계값이 실측에서 왔다는 것과
> 임계값을 통과했다는 것은 다르다. 이 구분을 잃으면 조사 결과가 슬금슬금 부풀어 오른다.

#### 대조군이 눈금을 맞춰 준다 — 그리고 **게이트 2의 "기하적 불가" 판정을 흔든다**

| 대조군 | 이번 | 게이트 2 실측 |
|---|---:|---:|
| `duck_lamp` (root 기준, 11.9 cm) | 10/10 | 7/10 |
| **`block_red` (root 기준, 2.5 cm)** | **6/10** | **1/10** |
| `block_red` (com 기준) | 2/10 | — |

duck_lamp 이 7/10 → 10/10 으로 오른 것은 **이번 그리드가 배치 후보를 3개로 넓혔기
때문**이다(게이트 2는 `x=0.03` 하나만 훑었다). 방향이 일치하므로 도구의 눈금은 맞다.

> ⚠️ **그런데 같은 이유로 `block_red` 가 1/10 → 6/10 이 됐다.** 게이트 2가 이 블록을
> 두고 내린 판정은 "사람 크기 손에 너무 작아 **3지 접촉이 기하적으로 성립하지 않는다**"
> 였고, 그것이 기본 학습 물체를 duck_lamp 으로 바꾼 근거였다(§10 발견 6).
>
> 이번 실측은 그 판정이 **기하의 문제가 아니라 배치의 문제**였음을 보인다 — 배치 후보를
> 3개로 넓히자 6/10 시드에서 성공 궤적이 존재한다. 같은 함정을 paper_cup 에서 이미
> 한 번 밟았다(배치만 바꿔 30 % → 100 %).
>
> **다만 뒤집힌 것은 "불가능"이지 "쉽다"가 아니다.** 성공 상한이 낮고(최선 고정궤적
> 4/10, tips 최대 3, 낙하 43 %) duck_lamp 급 안정성과는 거리가 멀다. 판정은 **"경계"** 다.
> 게이트 2의 학습 실패 원인 분석(물체 크기) 자체는 유효하다 — 2.5 cm 는 여전히 어렵다.

**기준점을 바꾸면 결과가 또 달라진다 — 배치 민감도가 그만큼 크다.** `block_red[com]`
2/10 은 root(모서리) 기준의 6/10 보다 **낮다**. com 이 나쁜 것이 아니라, 이번 그리드가
**손바닥 x 축만** 훑었기 때문이다(y·z 고정). 물체 중심을 x 축 위에 정확히 놓는 것이 이
블록에는 최적이 아니었다. 이 민감도 자체가 **`place` 상수를 없애야 하는 이유의 실측
근거**다 — 사람이 축 하나를 고르는 것으로도 성립률이 3배 움직인다.

#### 이 측정을 하려고 env 에 넣은 것 두 가지 (둘 다 opt-in, 기본 동작 불변)

| 설정 | 왜 필요했나 |
|---|---|
| `object_actor` | `shape_box` 는 **도형 12조각 + 몸체 + 뚜껑이 한 프리팹**이다. 그대로 올리면 env 가 첫 actor(`star`)를 조용히 집고 나머지 13개가 같은 자리에 겹쳐 생성된다. 지정한 조각만 남기고 나머지는 `scene.destroy_actor` 로 지운다. 조각을 지정하지 않으면 오류로 멈춘다 |
| `object_ref="com"` | **actor root 는 메시 원점이라 asset 마다 제각각이다**(실측): block_red 모서리 `(1.25, 1.25, 1.25) cm`, paper_cup 바닥면 `(0, 0, 5.6) cm`, shape_box 조각 한쪽 면 `(0, 2.0, 0) cm`. env 는 이 root 를 관찰의 물체 위치·성공 거리 판정·스폰 배치에 **전부** 쓴다. 4 cm 물체에서 2 cm 오차는 배치 confound 다. rigid actor 는 `use_recentering=False` 라 root 가 질량중심으로 옮겨지지 않는다 |

> **기본값은 `root` 그대로다.** 게이트 0~4 의 모든 실측이 root 조건이라 기본을 바꾸면
> 비교가 끊긴다. 변경 전 코드와 **결정론적 롤아웃 누적 보상이 소수점 9자리까지 동일**함을
> 확인했다(hand20 / arm_hand26(palm) / arm_hand26(workspace) / paper_cup × 고정폐쇄·무작위 정책).
>
> ⚠️ `object_ref="com"` 은 **rigid 전용**이다. `get_rigid_center_of_mass_local()` 은 soft
> actor(duck_lamp)에서도 값을 돌려주지만 soft 는 recentering 이 켜져 있어 root 의 의미가
> 다르다 — soft 에서 확실히 실패하는 `get_center_of_mass_transform()` 으로 걸러 거부한다.

#### 재현 조건과 한계

조건: `control_mode="hand20"`(게이트 2와 같은 손 단독), `spawn_mode="palm"`,
`episode_seconds=1.0`, 시드 9000~9009, **리셋 회전 identity**(프리팹이 조각마다 갖고 있는
회전은 쓰지 않는다), 도형은 `object_ref="com"`, 대조군은 과거 조건인 `root`.

한계는 네 가지다 — 이 수치를 **정책 성능으로 읽으면 안 된다**:

1. **오라클은 개루프다.** "성공 궤적이 존재한다"이지 "정책이 배운다"가 아니다.
2. **자세가 하나다.** 조각을 identity 회전으로만 놓고 쟀다. 눕힌 자세·세운 자세는 미측정.
3. **배치를 x 축만 훑었다.** y·z 는 고정(`0, 0.04`). block_red[com] 이 보여주듯 나머지
   축에 더 좋은 지점이 있을 수 있다.
4. **성공 = 지문 3개 + 파지중심 6 cm 이내이고 들기는 없다.** 게이트 2의 판정을 그대로 썼다.
...
> ⚠️ **2026-09-14 정정 — "기하적으로 불가"는 과했다.** 배치 후보를 3개로 넓히자
> `block_red` 의 성공 궤적 존재 시드가 1/10 → **6/10** 이 됐다("shape_box 오라클 스윕" 절).
> 대조군으로 남겨 둔 판단과 "2.5 cm 는 어렵다"는 결론은 유효하지만, **불가능이 아니라
> 배치에 극도로 민감한 것**이다. 이 표의 `block_red`·`sphere` 0 % 도 기본 배치
> `place=(0.04, 0, 0.04)` 하나에서 나온 값이라는 점을 함께 읽어야 한다.
...
| | OS | CPU | GPU | RAM | env runner 기본값 |
|---|---|---|---|---|---|
| **회사** (주 작업, 성능 판정 기준) | Windows | Ryzen 5 7600 — **6C/12T** | **RTX 2080 8 GB** (Turing, sm_75) | 32 GB | **8** |
| 집 | Windows | Ryzen 7 7800X3D — 8C/16T | RTX 4070 Ti 12 GB (Ada) | 32 GB | 12 |
| **개인 노트북** (2026-09-10 추가) | **Ubuntu 24.04.4 LTS** | Core Ultra 5 225H — **14C/14T** (HT 없음) | **없음** — Intel Arc 내장 (Arrow Lake-P) | **16 GB** | **10** |

> 로드맵 v5·v14가 "학습 머신 RTX 4070 Ti / 7800X3D"로 적은 것은 **집 머신**이다.
> 회사 머신이 주 작업 환경이고 코어·VRAM이 더 작다 — **성능 판정의 기준은 회사 머신이다.**

#### 개인 노트북 (2026-09-10 추가) — 앞의 두 대와 전제가 다르다

기기는 LG gram 16 `16ZD90TP-GX56K`. 실측 사양:

| 항목 | 값 | 확인 방법 |
|---|---|---|
| CPU | Intel Core Ultra 5 225H — 14코어 / **14스레드** (P 4 @4.9 GHz + E 8 @4.4 GHz + **LPE 2 @2.5 GHz**) | `lscpu`, `lscpu --all --extended` |
| GPU | Intel Arc 내장 (Arrow Lake-P, PCI `8086:7d51`) — **CUDA 없음** | `lspci` 의 VGA 항목, `nvidia-smi` 없음 |
| RAM | 16 GB (가용 ≈15 GiB) + swap 4 GB | `free -h` |
| 디스크 | NVMe 233 GB (여유 ≈93 GB — 클론 1.2 GB + venv 약 4 GB 설치 후) | `df -h /` |
| OS | Ubuntu 24.04.4 LTS (커널 7.0.0-28-generic) | `/etc/os-release` |
| Python | 시스템 `3.12.3` (`/usr/bin/python3.12`), `3.10.20` (`/usr/bin/python3.10`), pyenv `3.10.12` | `python3.12 --version` |

**이 머신에서 깨지는 기존 전제 3가지 — 전부 대응해 뒀다:**

1. **CUDA GPU가 없다.** 위 §7 의 `RTAUTO_SUPERDEX_GPUS_PER_LEARNER` 기본값 `1` 은
   "두 머신 모두 CUDA GPU가 있다"를 전제로 정한 값이다. 여기서는 Ray 가 GPU 를 못 잡으므로
   이 머신의 `.env` 에 **`RTAUTO_SUPERDEX_GPUS_PER_LEARNER=0`** 을 넣었다(원칙 1 — 머신마다
   다른 값은 `.env`). torch 도 CUDA 휠이 아니라 **CPU 휠**을 쓴다
   (`--index-url https://download.pytorch.org/whl/cpu`, 실측 `torch 2.14.0+cpu`).
   `torch.cuda.is_available()` 가 **`False` 인 것이 이 머신의 정상 상태**다.
2. **RAM 이 절반(16 GB)이다.** env runner 기본값은 `os.cpu_count()-4` = **10** 으로
   나오는데, 이는 스레드 수에서만 파생된 값이고 메모리를 보지 않는다. 앞의 두 대(32 GB)에서
   통하던 runner 수가 여기서는 메모리로 먼저 막힐 수 있다 — OOM 이 나면 `.env` 의
   `RTAUTO_SUPERDEX_ENV_RUNNERS` 를 낮춰 대응한다(코드 기본값을 고치지 않는다).
3. **유일한 Linux 머신이다.** venv 활성화가 `superdex/.venv/bin/activate`(Windows 는
   `Scripts/Activate.ps1`), 클론 경로가 `/home/lkb/workspace/project_superdex`
   (회사는 `D:/workspace/project_superdex`). 문서의 **bash 블록**을 쓴다.
   ⚠️ **Studio(`superdex-studio`) 의 SDF bake 는 GUI 앱**이라 게이트 3-2 절차는 이 머신에서도
   사람이 해야 한다 — Linux 에서의 동작은 아직 확인하지 않았다.

**혼합정밀도 논의는 이 머신에 해당하지 않는다.** 아래 RTX 2080 bf16 경고는 CUDA learner
이야기다. 개인 노트북은 CPU learner 라 bf16/fp16 선택 자체가 없다.

> **성능 판정 기준은 바뀌지 않는다.** 개인 노트북의 throughput 을 게이트 판정·회귀 기준에
> 쓰지 않는다 — CPU learner + 저전력 코어 2개 포함이라 회사 머신과 비교 대상이 아니다.
> 이 머신의 용도는 **코드·문서 작업과 소규모 검증**이다.

**부트스트랩 실측 (2026-09-10, 이 머신에서 처음부터 수행):** `python3.12 -m venv` →
CPU torch → `pip install -r requirements-superdex.txt` → `project_superdex` `stable`
클론(커밋 `1d71509`, 1.2 GB) → `.env` 2줄 → `gate3_setup_asset_root.py` 1회.
결합 bot 로드 **통과 (links=41, joints=41)**. `pip check` 깨끗.
해석된 버전이 회사 머신과 동일했다(`ray 2.58.0`, `onnx 1.22.0`, `onnxruntime 1.29.0`) —
그래서 이 셋을 `requirements-superdex.txt` 에 `==` 로 **핀했다**(§9 R3 해소).
...
> ⚠️ **2026-09-14 정정.** 위 굵은 문장은 **과했다.** 이 표의 `block_red` 1/10 은 배치를
> `x=0.03` **하나만** 훑은 값이다. 2026-09-14 스윕에서 배치 후보를 0.02/0.03/0.04 로 넓히니
> **6/10** 이 됐다 — 3지 접촉이 성립하는 배치가 실제로 존재한다. 따라서 정확한 서술은
> "기하적으로 불가능하다"가 아니라 **"배치에 극도로 민감하고 성공 상한이 낮다"** 이다.
> v1~v4 학습 실패의 원인이 물체 크기였다는 이 절의 결론 자체는 유효하다(그 학습들도
> 같은 배치 하나를 썼다). 상세는 "shape_box 오라클 스윕 (2026-09-14)" 절.
