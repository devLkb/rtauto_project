...
>
> ### 🛑 다음 착수점 — 여기서 이어가라
> 1. **`place` 상수 제거 (최우선).** 현행 환경은 물체 스폰 위치를 **사람이 물체마다 맞춘
>    상수**로 쓴다. 게이트 4 에서 이 값이 성공을 지배했다(paper_cup 배치만 바꿔 30 %→100 %).
>    범용 파지 목적에서는 **시스템이 스스로 정해야** 하므로 이건 편의가 아니라 **결함**이다.
>    팔 6축이 선행 조건이었고 지금 충족됐다 — 물체를 작업면 위 무작위로 놓고 teacher 가
>    스스로 접근하게 한다.
