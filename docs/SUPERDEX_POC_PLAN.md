...
> 3. **물체 다양성 확보 — 전수 조사 완료. 다음은 shape_box 12종 파지 실험이다.**
>    `survey_object_assets.py` 로 공식 트리를 전부 쟀다: **확실히 쓸 수 있는 형상은 3종**
>    (duck_lamp·paper_cup·shapebox_body), **미검증 구간 14종**(대부분 shape_box 도형 12종,
>    3.8~5.0 cm). 2.5 cm(실패)와 9.3 cm(성공) 사이는 아무도 측정하지 않았으므로
>    **불가로 단정하지 않았다.** 그 12종이 잡히면 로스터가 3 → 15종이 된다.
>    👉 `gate2_oracle_search.py` 방식으로 12종 파지 성립 여부부터 측정하라 — 싸고 결정적이다.
...
|---|---|---|
| 물체 안착 높이 | 로봇에서 떨어뜨려 정착시킨 뒤 root z 를 읽는다 (rigid actor 는 local node 좌표를 못 준다 — 실측 확인) | **0.0291 m** |
...
**측정 방법.** 런타임에서는 AABB 를 못 읽는다(rigid actor 에 local node 좌표 컴포넌트가
없다 — 실측). 그래서 프리팹이 참조하는 **충돌 메시(`shape`, `.mochi.h5` 의
...
실측으로 아는 것은 두 점뿐이다: **2.5 cm 는 3지 접촉이 기하적으로 성립하지 않았고**
(게이트 2), **9.3 cm·11.9 cm 는 성립했다**. 그 사이 3~5 cm 구간은 **아무도 측정하지
않았다.** 하나의 임계값으로 잘라 버리면 `shape_box` 의 12가지 도형을 근거 없이 버리게
된다 — 그건 서로 형상이 다르고 크기가 고른, **일반화 학습에 가장 알맞은 집합**이다.

> 👉 **다음 실험 (싸고 결정적이다).** `shape_box` 12종에 오라클/스크립트 파지를 돌려
> 실제 성립 여부를 본다(`gate2_oracle_search.py` 방식). 잡히면 물체 로스터가
> **3종 → 15종**이 되어 물체 다양성 문제가 크게 풀린다. 안 잡히면 그때
> 외부 물체 셋(YCB 등) 도입이 확정된다. 어느 쪽이든 다음 단계가 정해진다.
