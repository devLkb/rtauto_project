...
> ## 2026-08-31 갱신 — 부채 ②·③ 해소, ④는 재확인
>
> 그리퍼 단독으로 Unity ↔ 실물 양방향 연결을 완료하며 아래가 바뀌었다. 상세는
> [`docs/SIM2REAL_ROADMAP.md`](../SIM2REAL_ROADMAP.md) §5 "2026-08-31 실물 연동 결과".
>
> - **부채 ② 해소** — `JOINT_ORDER`(항등)·`JOINT_SIGN`(전부 +1)·`JOINT_OFFSET_DEG`(0)를
>   하드웨어 설명서와 실측으로 확정했고, `JOINT_CLAMP`을 뭉뚱그린 ±130에서 채널별 실제
>   가동범위로 좁혔다. 다만 **부호를 한 번 잘못 뒤집어 관절을 하드 스톱에 1.5 A로
>   밀어붙인 사고가 있었다** — 설명서의 범위 표가 부호 없는 이동량이었던 게 원인.
>   진단은 반드시 전류를 함께 볼 것.
> - **부채 ③ 해소(일부)** — `GetReceivedGripperData`를 바인딩해 관절 실제각·전류·모듈
>   에러코드를 읽는다. 개루프에서 벗어났고, 명령 대비 추종을 `--track`으로 기록·요약한다.
>   `get_fingertip_sensor_data()`·`grasp()`·`manual_teach_mode()` 중 교시 모드는 `--teach`로
>   사용 중이고, 핑거팁 센서와 내장 파지는 여전히 미사용이다.
> - **부채 ④ 재확인** — Tesollo ROS2 패키지(`dg5f_ros2`)를 검토했으나 그리퍼가 클라이언트
>   1개만 허용하는 제약은 동일해 이득이 없었다. UDP 자체 프로토콜 유지가 여전히 합리적이다.
> - **부채 ① 미해소** — 여전히 ctypes 직접 바인딩이다. 다만 콜백 등록 누락으로 네이티브
>   크래시를 겪는 등, 공식 래퍼를 썼으면 피했을 비용이 실제로 발생했다.
