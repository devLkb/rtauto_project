...
🛑 **단, 이 "입력"이 `build_pose_dataset.py`가 만든 물체 기준(`frame="object"`)
좌표라면, 학습한 쪽과 공정하게 비교되는 것과는 별개로 **둘 다 실물 D405에서는 못 쓰는
입력**이다(2026-09-21 코드 리뷰, 아직 미해결 — 위 스크립트 docstring 참고). 두 방식이
같은 입력을 받는다는 것이 그 입력 자체가 실물에서 재현 가능하다는 뜻은 아니다.
