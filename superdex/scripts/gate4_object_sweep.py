...
1. **과제 자체가 이 배치에서 안 풀리는 실패.** 게이트 2 발견 6: 2.5 cm 블록은 지문이
   1~2개만 닿아 3지 접촉을 만들기 어렵다(오라클 그리드 탐색에서 성공 궤적이 존재하는
   시드 1/10). 이런 실패를 정책 일반화 실패로 세면 오판이다. 그래서 작은 물체들을
   **대조군으로 남겨 둔다** — 빼 버리면 "큰 물체만 골랐다"가 되고, 넣고 실패로 세면
   "일반화가 안 된다"는 오판이 된다.
   ⚠️ **2026-09-14 정정**: 그 1/10 은 배치 `x=0.03` 하나만 훑은 값이다. 배치 후보를
   0.02/0.03/0.04 로 넓히면 **6/10** 이 된다(`object_oracle_sweep.py`). 아래 `OBJECTS` 의
   `feasible=False` 는 **"기하적으로 불가능"이 아니라 "기본 배치에서 성립하지 않는다"**
   로 읽어라 — 그 구분이 바로 이 문서가 말하는 실패 2번(배치)과 같은 함정이다.
