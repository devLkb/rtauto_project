...
🛑 **미해결 — 이 파일이 쓰는 "물체 기준" 자체가 또 다른 누수다 (2026-09-21 코드
리뷰로 발견, 아직 안 고침).** 아래 `cloud_in_object_frame()`/`palm_pose_in_object_frame()`
가 점 구름·자세를 물체 기준으로 맞추는 데 `env.block.get_root_transform()`
(시뮬레이터만 아는 물체의 **정답 위치**)를 쓴다. §7-3이 막으려던 것("시뮬레이터만
아는 값을 입력에 섞지 않기")과 같은 종류의 문제가 좌표값 자체에 숨어 있는 것이다 —
입력 배열에 물체 이름이 안 보여도, 좌표가 이미 정답으로 정렬돼 있으면 누수다.
실물 D405는 물체의 정답 위치를 몰라서(그걸 풀려는 것이 이 프로젝트의 목적이다,
`CLAUDE.md`) 똑같은 입력을 절대 만들 수 없다 — 순환이다.

`superdex/scripts/test_pose_dataset.py` §5가 이제 `frame == "object"` 인 산출물을
**거부**하도록 고쳐져 있다(예전엔 반대로 요구했다). 즉 지금 이 스크립트가 만드는
데이터셋은 그 시험을 통과하지 못한다 — **의도된 동작**이다. 재설계 방향(카메라
기준으로, 점 구름과 자세를 같은 카메라 시점에 묶기 등)은 이 코드 리뷰의 최종 보고서
"D-4 입력 좌표계 수정 원칙" 절 참고. 시뮬레이터를 실제로 돌려 검증해야 하는 작업이라
이번 리뷰에서는 코드를 바꾸지 않았다.
