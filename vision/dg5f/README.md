...
| `camera_picker.py` | **카메라 선택 창**(tkinter). 카메라 목록과 각 카메라의 사진을 보여 주고 하나를 고르게 한다. `vision_node_dg5f.py --pick` 또는 `.env`의 `RTAUTO_VISION_CAMERA_INDEX=ask`로 뜬다. 창을 못 띄우는 환경에서는 터미널로 묻는다. **2026-09-23부터 예전에 찾아 둔 목록(`camera_caps_cache.json`)을 카메라를 열지 않고 바로 띄운다**(실측 4.6초 → 0.25초). 새로 꽂은 카메라는 창의 "다시 찾기"(터미널에선 `r`)로 찾는다(사진 포함, 예전 방식). 목록에서 고른 카메라가 빠져 있으면 알아서 새로 찾아 다시 묻는다 |
